    ...
    def call_models(p,n_f,seq_length,y_len,model_name='LSTM') :
        ...
        for i in range(len(path)) :
            ...
            if model_name == 'LSTM' :
                ...
                param_dict[names[i]] = GRU(n_features=n_f, n_hidden=128, seq_len=seq_length, y_length=y_len)
            elif model_name == 'B_LSTM' :
                param_dict[names[i]] = B_LSTM(n_features=n_f, n_hidden=128, seq_len=seq_length, y_length=y_len)
            elif model_name == 'BiLSTM' :
                param_dict[names[i]] = BiLSTM(n_features=n_f, n_hidden=128, seq_len=seq_length, y_length=y_len)
...
class AgriDataSet(object) :
    ...
    def __init__(self, price_path,weather_path) :
        ...
        self.price = price[price[2]==k].groupby([0,1]).mean().pivot_table(index=0,columns=1,values=3).mean(axis=1)
    ...
    def _make_pr_dr_var(self,period,list_of_roll) :
        ...
        price_decomp = seasonal_decompose(self.price, model='additive',period=period,extrapolate_trend='freq')
